...
const logger = createHookLogger('useSocketManager');
...
export const useSocketManager = ({
  ...
  useEffect(() => {
    if (!namespace) {
      logger.debug('Namespace is required');
      return;
    }
    ...
    if (managers.has(socketUrl)) {
      ...
      logger.debug('Reusing existing Manager instance');
    } else {
      logger.debug('Creating new Manager instance');
    ...
    logger.debug(`Socket instance obtained (id might be undefined initially)`);

    const handleConnect = () => {
      // Log ID here, where it's guaranteed to be available
      logger.debug(`Connected: ${newSocket.id}`);
      setIsConnected(true);
    ...
    const handleDisconnect = (reason: ClientSocket.DisconnectReason) => {
      // Log ID here
      logger.debug(`Disconnected: ${newSocket.id}`, { reason });
    ...
    const handleConnectError = (err: Error) => {
      // Log ID here
      logger.debug(`Connection Error: ${newSocket.id}`, err);
      setIsConnected(false);
      setIsConnecting(false);
    ...
    } else if (autoConnect && !isConnecting) {
      // Check !isConnecting to avoid multiple connect calls
      logger.debug(`Auto-connecting socket (id might be undefined initially)`);
    ...
    return () => {
      ...
      logger.debug(`Cleaning up socket: ${newSocket.id}`);
      newSocket.off('connect', handleConnect);
      newSocket.off('disconnect', handleDisconnect);
      newSocket.off('connect_error', handleConnectError);
