...
const RetroBoardHistory: FC = () => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      ...
      setIsLoading(true);
      try {
        const response = await fetch(`${config.apiUrl}/history/retro/${boardId}`, {
          headers: {
  ...
  const getCardsForColumn = (columnId: string) => {
    ...
    return selectedSnapshot.snapshot.cards.filter(card => card.column_id === columnId);
  };
  ...
  if (history.length === 0) {
    return (
      <Box textAlign="center" p={8}>
        ...
        />
        <Heading size="lg" mb={4}>No History Available</Heading>
        <Text mb={6}>This board doesn't have any saved history snapshots yet.</Text>
        <Button 
          as={RouterLink} 
          to={`/retro/${boardId}`}
          colorScheme="blue"
    ...
    <Box>
      ...
      >
        ...
        <Box 
          ...
          minH="400px"
        >
          <Heading size="md" mb={4}>Snapshots</Heading>
          ...
          {selectedSnapshot ? (
            <VStack align="stretch" spacing={6}>
              ...
              <SimpleGrid columns={{ base: 1, md: 3 }} spacing={4}>
                ...
                <Box>
                  <Heading size="sm" mb={3} p={2} bg="green.100" color="green.800" borderRadius="md">
                    What Went Well
                  </Heading>
                  <VStack align="stretch" spacing={3}>
                    {getCardsForColumn('went-well').map(card => (
                      <Card key={card.id} size="sm" variant="outline">
                        <CardHeader pb={0}>
                          <Badge fontSize="xs" colorScheme="green">{card.author_name}</Badge>
                        </CardHeader>
                        <CardBody pt={2}>
                ...
                <Box>
                  <Heading size="sm" mb={3} p={2} bg="red.100" color="red.800" borderRadius="md">
                    Needs Improvement
                  </Heading>
                  <VStack align="stretch" spacing={3}>
                    {getCardsForColumn('to-improve').map(card => (
                      <Card key={card.id} size="sm" variant="outline">
                        <CardHeader pb={0}>
                          <Badge fontSize="xs" colorScheme="red">{card.author_name}</Badge>
                        </CardHeader>
                        <CardBody pt={2}>
                          <Text fontSize="sm">{card.text}</Text>
                        </CardBody>
                ...
                {/* Action Items Column */}
                <Box>
                  <Heading size="sm" mb={3} p={2} bg="blue.100" color="blue.800" borderRadius="md">
                    Action Items
                  </Heading>
                  <VStack align="stretch" spacing={3}>
                    {getCardsForColumn('action-items').map(card => (
                      <Card key={card.id} size="sm" variant="outline">
                        <CardHeader pb={0}>
                          <Badge fontSize="xs" colorScheme="blue">{card.author_name}</Badge>
                        </CardHeader>
                        <CardBody pt={2}>
                          <Text fontSize="sm">{card.text}</Text>
                        </CardBody>
